  ...
  return (
    <header className="flex h-10 items-center justify-between border-b border-border bg-card/80 backdrop-blur-md px-3 py-1.5 z-50">
      <div className="flex items-center gap-1.5">
        <button
          onClick={onMenuClick}
          className={`relative p-1.5 rounded-sm transition-colors duration-200 outline-none focus-visible:ring-1 focus-visible:ring-primary/50 active:scale-95 ${
            isSidebarOpen
        ...
        >
          ...
          <span className="absolute inset-0 -m-1.5" aria-hidden="true" />
        </button>
        ...
        <div className="flex items-center gap-2.5 ml-1">
          ...
          <h1 className="font-mono text-xs font-semibold text-foreground tracking-tight select-none">
            nodepad
          </h1>
          {activeProjectName && (
            <div className="flex items-center gap-2 ml-1">
              <span className="text-muted-foreground/20 font-mono text-[10px]">/</span>
              <span className="font-mono text-[10px] text-muted-foreground font-semibold uppercase tracking-[0.2em]">{activeProjectName}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        {blockCount > 0 && (
          <div className="flex items-center gap-4">
            <span className="font-mono text-[10px] text-muted-foreground/40 font-semibold uppercase tracking-wider tabular-nums">
            ...
            {activity.errors > 0 && (
              <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-sm bg-destructive/10">
                ...
                <span className="font-mono text-[10px] text-destructive font-semibold tabular-nums">
                ...
                <div className="flex items-center gap-3">
                  {typeCounts.map(([type, count]) => {
                    ...
                    return (
                      <span
                        key={type}
                        className="font-mono text-[10px] font-semibold uppercase tracking-tighter tabular-nums"
        ...
        <div className="flex items-center gap-2 border-l border-white/5 pl-4 ml-4">
          {/* Model indicator */}
          {modelLabel && (
            <span className="font-mono text-[10px] text-muted-foreground/60 uppercase tracking-wider px-1.5">
              {modelLabel}
            </span>
          )}
          ...
          >
            ...
            {ghostNoteCount > 0 && !isGhostPanelOpen && (
              <span className="absolute -top-1 -right-1 flex size-3.5 items-center justify-center rounded-full bg-primary font-mono text-[10px] font-semibold text-primary-foreground tabular-nums">
                {ghostNoteCount}
              </span>
            )}
          </button>
          ...
          <div className="relative">
            <button
              ...
              className="relative p-1.5 rounded-sm transition-colors duration-200 hover:bg-secondary text-muted-foreground/40 hover:text-foreground active:scale-95"
              title="About nodepad"
            >
              <span className="font-mono text-[11px] font-semibold leading-none">?</span>
              <span className="absolute inset-0 -m-1.5" aria-hidden="true" />
            </button>
            ...
            <AnimatePresence initial={false}>
              {showHelpTooltip && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-2.5 z-[300] w-48 rounded-sm bg-primary text-primary-foreground shadow-elevated pointer-events-none select-none"
                >
                  {/* Arrow pointing up toward the ? button */}
                  <div className="absolute -top-1.5 right-2.5 size-3 rotate-45 bg-primary rounded-[2px]" />
                  <div className="relative px-3 py-2.5">
                    <p className="text-[11px] font-medium leading-snug text-balance">
